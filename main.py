...
def main():
    ...
    if st.button("Exploratory Data Analysis"):
        preprocessor = train_Preprocessor(
            file_object= "Training_Logs/Training_Preprocessing_Log.txt",data_path = 'Training_Data_FromDB/Training_Data.csv',result_dir= 'Intermediate_Train_Results/')
        preprocessor.eda(target_col = 'Output')
        st.success(
            "This step of the pipeline has been completed successfully. Check the local files for more details.")
    if st.button("Training Data Preprocessing"):
        ...
        preprocessor.data_preprocessing(
            col_drop_path= 'Columns_Drop_from_Original.csv', index_col= 'Wafer', target_col = 'Output')
        st.success(
            "This step of the pipeline has been completed successfully. Check the local files for more details.")
